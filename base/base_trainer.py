...
class BaseTrainer_semiseg:
    def __init__(self, model, resume, config, iters_per_epoch, train_logger=None, gpu=None, test=False):
        ...
        # OPTIMIZER
        self.optimizer_l = get_instance(torch.optim, 'optimizer', config, trainable_params_l) # trainable_params should be obtained before wraping the model with DistributedDataParallel
        self.optimizer_r = get_instance(torch.optim, 'optimizer', config, trainable_params_r) 
        ...
        self.lr_scheduler_l = getattr(utils.lr_scheduler, config['lr_scheduler'])(optimizer=self.optimizer_l, num_epochs=self.epochs, 
                                        iters_per_epoch=iters_per_epoch)
        self.lr_scheduler_r = getattr(utils.lr_scheduler, config['lr_scheduler'])(optimizer=self.optimizer_r, num_epochs=self.epochs, 
                                        iters_per_epoch=iters_per_epoch)
    ...
    def _save_checkpoint(self, epoch, save_best=False):
        state = {
            'arch': type(self.model.branch1).__name__,
            'epoch': epoch,
            'state_dict': self.model.branch1.state_dict(),
            'monitor_best': self.mnt_best,
            'config': self.config
        }
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        try:
            self.model.branch1.load_state_dict(checkpoint['state_dict'])
        except Exception as e:
            print(f'Error when loading: {e}')
            self.model.branch1.load_state_dict(checkpoint['state_dict'], strict=False)
